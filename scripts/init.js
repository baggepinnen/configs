...
atom.packages.onDidActivateInitialPackages(() => {
  const jc = atom.packages.getLoadedPackage('julia-client')
  if (!jc) return
  atom.commands.add('atom-workspace', {
    'julia-client:toggle-plot-pane': () => {
      const prev = atom.config.get('julia-client.uiOptions.usePlotPane')
      atom.config.set('julia-client.uiOptions.usePlotPane', !prev)
    },
  })
  atom.keymaps.add(
    'init.js', { 'atom-workspace': { 'ctrl-j alt-p': 'julia-client:toggle-plot-pane' } }, 1,
  )
})
